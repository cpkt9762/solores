...
pub fn generate_claude_md(
    ...
    // 使用include_str!直接包含CLAUDE.md模板
    let template_content = include_str!("../templates/common/claude.md.jinja");
    
    let tmpl = env.template_from_str(template_content)
        .map_err(|e| SoloresError::TemplateError {
            template_name: Some("common/claude.md.jinja".to_string()),
            message: format!("模板解析失败: {}", e),
            context: Some("解析CLAUDE.md模板".to_string()),
        })?;
    ...
    let rendered = tmpl.render(context)
        .map_err(|e| SoloresError::TemplateError {
            template_name: Some("common/claude.md.jinja".to_string()),
            message: format!("模板渲染失败: {}", e),
            context: Some("渲染CLAUDE.md模板".to_string()),
        })?;
    
    let output_path = output_dir.join("CLAUDE.md");
    fs::write(&output_path, rendered)
        .map_err(|e| SoloresError::FileOperationError {
            operation: "write CLAUDE.md".to_string(),
            path: output_path.display().to_string(),
            current_dir: std::env::current_dir().ok().map(|p| p.display().to_string()),
            resolved_path: None,
            source: e,

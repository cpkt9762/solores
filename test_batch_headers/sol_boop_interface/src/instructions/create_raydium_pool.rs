...
///Accounts struct for the create_raydium_pool instruction
...
///Public keys struct for the create_raydium_pool instruction
...
impl From<&[String]> for CreateRaydiumPoolKeys {
...
{
    fn from(arr: &'me [AccountInfo<'info>; CREATE_RAYDIUM_POOL_IX_ACCOUNTS_LEN]) -> Self {
        Self {
            cpmm_program: &arr[0],
            amm_config: &arr[1],
            authority: &arr[2],
...
pub const CREATE_RAYDIUM_POOL_IX_DISCM: [u8; 8] = [65, 45, 119, 77, 204, 178, 84, 2];
        ...
        if maybe_discm != CREATE_RAYDIUM_POOL_IX_DISCM {
            return Err(std::io::Error::new(
                ...
                format!(
                    ...
                    CREATE_RAYDIUM_POOL_IX_DISCM, maybe_discm
                ),
            ));
        }
        Ok(Self)
...
pub fn create_raydium_pool_ix(keys: CreateRaydiumPoolKeys) -> std::io::Result<Instruction> {
    create_raydium_pool_ix_with_program_id(crate::ID, keys)
}
...
pub fn create_raydium_pool_invoke(accounts: CreateRaydiumPoolAccounts<'_, '_>) -> ProgramResult {
    create_raydium_pool_invoke_with_program_id(crate::ID, accounts)
}
...
pub fn create_raydium_pool_invoke_signed(
    ...
    create_raydium_pool_invoke_signed_with_program_id(crate::ID, accounts, seeds)
}

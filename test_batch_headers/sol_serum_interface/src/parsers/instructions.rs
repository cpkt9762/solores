...
pub fn parse_instruction(data: &[u8]) -> Result<SerumInstruction, std::io::Error> {
    if data.is_empty() {
    ...
    match discriminator {
        0u8 => {
            ...
            Ok(SerumInstruction::InitializeMarket(args))
        }
        1u8 => {
            let args = NewOrderIxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::NewOrder(args))
        }
        2u8 => {
            let args = MatchOrdersIxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::MatchOrders(args))
        }
        3u8 => {
            let args = ConsumeEventsIxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::ConsumeEvents(args))
        }
        4u8 => {
            let args = CancelOrderIxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::CancelOrder(args))
        }
        5u8 => Ok(SerumInstruction::SettleFunds),
        6u8 => {
            let args = CancelOrderByClientIdIxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::CancelOrderByClientId(args))
        }
        7u8 => Ok(SerumInstruction::DisableMarket),
        8u8 => Ok(SerumInstruction::SweepFees),
        9u8 => {
            let args = NewOrderV2IxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::NewOrderV2(args))
        }
        10u8 => {
            let args = NewOrderV3IxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::NewOrderV3(args))
        }
        11u8 => {
            let args = CancelOrderV2IxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::CancelOrderV2(args))
        }
        12u8 => {
            let args = CancelOrderByClientIdV2IxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::CancelOrderByClientIdV2(args))
        }
        13u8 => {
            let args = SendTakeIxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::SendTake(args))
        }
        14u8 => Ok(SerumInstruction::CloseOpenOrders),
        15u8 => Ok(SerumInstruction::InitOpenOrders),
        16u8 => {
            let args = PruneIxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::Prune(args))
        }
        17u8 => {
            let args = ConsumeEventsPermissionedIxArgs::deserialize(&mut ix_data)?;
            Ok(SerumInstruction::ConsumeEventsPermissioned(args))
        }
...
mod tests {
    ...
    fn test_initialize_market_consistency() {
        ...
        match parse_instruction(&test_data) {
            Ok(SerumInstruction::InitializeMarket(_)) => {}
    ...
    fn test_new_order_consistency() {
        ...
        test_data.extend_from_slice(&test_args.try_to_vec().unwrap());
        match parse_instruction(&test_data) {
            Ok(SerumInstruction::NewOrder(_)) => {}
            Ok(_) => panic!("Discriminator matched wrong instruction type"),
            Err(e) => {
                let error_msg = e.to_string();
                assert!(
    ...
    fn test_cancel_order_consistency() {
        ...
        test_data.extend_from_slice(&test_args.try_to_vec().unwrap());
        match parse_instruction(&test_data) {
            Ok(SerumInstruction::CancelOrder(_)) => {}
            Ok(_) => panic!("Discriminator matched wrong instruction type"),
            Err(e) => {
                let error_msg = e.to_string();
                assert!(
    ...
    fn test_disable_market_consistency() {
        ...
        let parsed = parse_instruction(&test_data).expect("Failed to parse instruction");
        match parsed {
            SerumInstruction::DisableMarket => {}
            _ => panic!("Parsed instruction has wrong type"),
        }
    }
    #[test]
    ...
    fn test_new_order_v_2_consistency() {
        ...
        test_data.extend_from_slice(&test_args.try_to_vec().unwrap());
        match parse_instruction(&test_data) {
            Ok(SerumInstruction::NewOrderV2(_)) => {}
            Ok(_) => panic!("Discriminator matched wrong instruction type"),
            Err(e) => {
                let error_msg = e.to_string();
                assert!(
    ...
    fn test_new_order_v_3_consistency() {
        ...
        test_data.extend_from_slice(&test_args.try_to_vec().unwrap());
        match parse_instruction(&test_data) {
            Ok(SerumInstruction::NewOrderV3(_)) => {}
            Ok(_) => panic!("Discriminator matched wrong instruction type"),
            Err(e) => {
                let error_msg = e.to_string();
                assert!(
    ...
    fn test_cancel_order_v_2_consistency() {
        ...
        test_data.extend_from_slice(&test_args.try_to_vec().unwrap());
        match parse_instruction(&test_data) {
            Ok(SerumInstruction::CancelOrderV2(_)) => {}
            Ok(_) => panic!("Discriminator matched wrong instruction type"),
            Err(e) => {
                let error_msg = e.to_string();
                assert!(
    ...
    fn test_send_take_consistency() {
        ...
        test_data.extend_from_slice(&test_args.try_to_vec().unwrap());
        match parse_instruction(&test_data) {
            Ok(SerumInstruction::SendTake(_)) => {}
            Ok(_) => panic!("Discriminator matched wrong instruction type"),
            Err(e) => {
                let error_msg = e.to_string();
                assert!(
    ...
    fn test_close_open_orders_consistency() {
        ...
        let parsed = parse_instruction(&test_data).expect("Failed to parse instruction");
        match parsed {
            SerumInstruction::CloseOpenOrders => {}
            _ => panic!("Parsed instruction has wrong type"),
        }
    }
    #[test]
    fn test_init_open_orders_consistency() {
        ...
        let parsed = parse_instruction(&test_data).expect("Failed to parse instruction");
        match parsed {
            SerumInstruction::InitOpenOrders => {}
            _ => panic!("Parsed instruction has wrong type"),
        }
    }
    #[test]
    ...
    fn test_discriminator_recognition() {
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::InitializeMarket(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::NewOrder(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::MatchOrders(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::ConsumeEvents(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::CancelOrder(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::CancelOrderByClientId(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::NewOrderV2(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::NewOrderV3(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::CancelOrderV2(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::CancelOrderByClientIdV2(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::SendTake(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::Prune(parsed_args) => {
                    assert_eq!(
        ...
        match result {
            Ok(instruction) => match instruction {
                SerumInstruction::ConsumeEventsPermissioned(parsed_args) => {
                    assert_eq!(

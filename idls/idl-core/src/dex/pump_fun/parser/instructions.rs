...
use sol_pumpfun_interface::{
    parsers::instructions::{parse_instruction, PumpfunInstruction},
    ID as PUMPFUN_ID,
};
...
impl Parser for InstructionParser {
    ...
    type Output = PumpfunInstruction;

    fn id(&self) -> std::borrow::Cow<str> { "PumpFun::InstructionParser".into() }

    fn prefilter(&self) -> Prefilter {
        Prefilter::builder()
            .transaction_accounts([PUMPFUN_ID])
            .build()
            .unwrap()
    }

    async fn parse(&self, ix_update: &InstructionUpdate) -> ParseResult<Self::Output> {
        if ix_update.program.equals_ref(PUMPFUN_ID.to_bytes()) {
            // 转换账户格式

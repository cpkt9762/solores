...
// 直接引用生成的Raydium AMM接口  
use sol_raydium_interface::{
    parsers::instructions::{parse_instruction, ProgramInstruction},
...
impl Parser for InstructionParser {
    ...
    type Output = ProgramInstruction;

    fn id(&self) -> std::borrow::Cow<str> { "Raydium::AmmInstructionParser".into() }

    fn prefilter(&self) -> Prefilter {
        Prefilter::builder()
            .transaction_accounts([RAYDIUM_AMM_ID])
            .build()
            .unwrap()
    }

    async fn parse(&self, ix_update: &InstructionUpdate) -> ParseResult<Self::Output> {
        if ix_update.program.equals_ref(RAYDIUM_AMM_ID.to_bytes()) {
            // 转换账户格式

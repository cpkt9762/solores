...
// Generated interface crates (always available)
pub mod crates {
    pub use sol_raydium_interface as raydium_amm;
    pub use sol_raydium_launchpad_interface as raydium_launchpad;
    pub use sol_pumpfun_interface as pump_fun;
    pub use sol_whirlpool_interface as orca_whirlpool;
}

// Re-export error types for convenience
pub use error::{IdlCoreError, ParseError, BoxedError, Result, ParseResult};
...
impl<const LEN: usize> TryFrom<&[u8]> for KeyBytes<LEN> {
    type Error = crate::error::KeyFromSliceError;
...
impl<const LEN: usize> KeyBytes<LEN> {
    ...
    /// returns an error.
    pub fn try_from_ref<T: AsRef<[u8]>>(key: T) -> std::result::Result<Self, crate::error::KeyFromSliceError> {
        key.as_ref().try_into()
    }
...
impl<const LEN: usize> FromStr for KeyBytes<LEN> {
    type Err = crate::error::KeyFromStrError<LEN>;
...
impl<const LEN: usize> TryFrom<&str> for KeyBytes<LEN> {
    type Error = crate::error::KeyFromStrError<LEN>;
...
impl<const LEN: usize> TryFrom<Cow<'_, str>> for KeyBytes<LEN> {
    ...
    fn try_from(value: Cow<str>) -> std::result::Result<Self, Self::Error> {
        value.parse()
    }
}
...
pub struct PrefilterBuilder {
    error: Option<IdlCoreError>,
    accounts: Option<HashSet<Pubkey>>,
    account_owners: Option<HashSet<Pubkey>>,
    transaction_accounts: Option<HashSet<Pubkey>>,
}

fn set_opt<T>(opt: &mut Option<T>, field: &'static str, val: T) -> std::result::Result<(), IdlCoreError> {
    if opt.is_some() {
        return Err(IdlCoreError::from(crate::error::PrefilterError::AlreadySet(field)));
    }
...
fn collect_pubkeys<I: IntoIterator>(it: I) -> std::result::Result<HashSet<Pubkey>, IdlCoreError>
where
    I::Item: AsRef<[u8]>,
{
    it.into_iter()
        .map(|p| {
            let p = p.as_ref();
            p.try_into()
                .map_err(|e| IdlCoreError::from(crate::error::PrefilterError::BadPubkey(p.to_vec(), e)))
        })
        .collect()
}
...
impl PrefilterBuilder {
    ...
    pub fn build(self) -> std::result::Result<Prefilter, IdlCoreError> {
    ...
    fn mutate<F: FnOnce(&mut Self) -> std::result::Result<(), IdlCoreError>>(mut self, f: F) -> Self {
...
// Re-export builtin parsers for convenience
pub use builtin_parsers::{
    ComputeBudgetParser, MemoParser, SplTokenParser, SplToken2022Parser, SystemProgramParser,
};

// Re-export runtime system
pub use runtime::{ParsedInstruction, ParsedResult, RuntimeBuilder, RuntimeConfig, SoloresRuntime};

// Re-export DEX parsers for convenience
pub use dex::{
    RaydiumAmmInstructionParser, RaydiumAmmAccountParser,
    RaydiumLaunchpadInstructionParser, RaydiumLaunchpadAccountParser,
    PumpFunInstructionParser, PumpFunAccountParser,
    OrcaWhirlpoolInstructionParser, OrcaWhirlpoolAccountParser,
};

// Convenient re-exports for direct access to generated crates
pub use crates::*;

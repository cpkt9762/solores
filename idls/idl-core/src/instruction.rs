...
pub enum ParseError {
    ...
    Base64Error(#[from] bs58::decode::Error),
}
...
impl InstructionUpdate {
    ...
    /// Parse an encoded confirmed transaction with status meta into a list of instructions.
    ///
    /// # Errors
    /// Returns an error if the transaction data cannot be decoded or parsed.
    pub fn parse_from_meta(
        encoded_transaction: &EncodedConfirmedTransactionWithStatusMeta,
        slot: u64,
    ) -> Result<Vec<Self>, ParseError> {
        // Use the comprehensive parser from the dedicated module
        crate::mata_parser::parse_encoded_confirmed_transaction(encoded_transaction, slot)
    }

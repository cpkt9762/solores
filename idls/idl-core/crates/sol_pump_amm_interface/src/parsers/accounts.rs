...
pub fn try_unpack_account(data: &[u8]) -> Result<PumpAmmAccount, std::io::Error> {
    match BondingCurve::from_bytes(data) {
        Ok(account) => return Ok(PumpAmmAccount::BondingCurve(account)),
    ...
    match GlobalConfig::from_bytes(data) {
        Ok(account) => return Ok(PumpAmmAccount::GlobalConfig(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match Pool::from_bytes(data) {
        Ok(account) => return Ok(PumpAmmAccount::Pool(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }

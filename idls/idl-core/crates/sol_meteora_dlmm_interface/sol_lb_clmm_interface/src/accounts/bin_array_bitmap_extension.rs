...
pub struct BinArrayBitmapExtension {
    ...
    pub positive_bin_array_bitmap: u8,
    ///Packed initialized bin array state for start_bin_index is negative
    pub negative_bin_array_bitmap: u8,
}
impl Default for BinArrayBitmapExtension {
    fn default() -> Self {
        Self {
            discriminator: BIN_ARRAY_BITMAP_EXTENSION_ACCOUNT_DISCM,
            lb_pair: Default::default(),
            positive_bin_array_bitmap: Default::default(),
            negative_bin_array_bitmap: Default::default(),
        }
    }
}
impl BinArrayBitmapExtension {
    pub const MEM_LEN: usize = std::mem::size_of::<Self>();
    pub const PACKED_LEN: usize = 56usize;

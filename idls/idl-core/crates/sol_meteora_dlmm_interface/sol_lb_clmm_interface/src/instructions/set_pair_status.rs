//! This file was automatically generated by Solores
//! Solana IDL to Rust interface generator
//!
//! GitHub: https://github.com/cpkt9762/solores
//!
//! Generated on: 2025-08-16
//!
//! DO NOT EDIT - This file is auto-generated
//! Changes will be lost when regenerating from IDL

//!Instruction: set_pair_status
#[allow(unused_imports)]
use solana_pubkey::Pubkey;
pub const SET_PAIR_STATUS_IX_DISCM: [u8; 8] =
    [67u8, 248u8, 231u8, 137u8, 154u8, 149u8, 217u8, 174u8];
pub const SET_PAIR_STATUS_IX_ACCOUNTS_LEN: usize = 2;
#[derive(borsh::BorshDeserialize, borsh::BorshSerialize, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct SetPairStatusIxData {
    pub discriminator: [u8; 8],
    pub status: u8,
}
impl Default for SetPairStatusIxData {
    fn default() -> Self {
        Self {
            discriminator: SET_PAIR_STATUS_IX_DISCM,
            status: Default::default(),
        }
    }
}
impl SetPairStatusIxData {
    pub fn new(status: u8) -> Self {
        Self {
            discriminator: Self::discriminator(),
            status,
        }
    }
    pub fn from_bytes(buf: &[u8]) -> std::io::Result<Self> {
        borsh::BorshDeserialize::deserialize(&mut &buf[..])
    }
    pub fn discriminator() -> [u8; 8] {
        SET_PAIR_STATUS_IX_DISCM
    }
    pub fn try_to_vec(&self) -> std::io::Result<Vec<u8>> {
        borsh::to_vec(self)
    }
    /// Manual JSON serialization
    #[cfg(feature = "serde")]
    pub fn to_json(&self) -> String {
        format!(
            "{{{}}}",
            [
                format!(
                    "\"discriminator\":[{}]",
                    self.discriminator
                        .iter()
                        .map(|x| x.to_string())
                        .collect::<Vec<_>>()
                        .join(",")
                ),
                format!("\"{}\":{}", "status", self.status)
            ]
            .join(",")
        )
    }
}
#[derive(Copy, Clone, Debug, PartialEq, Default)]
pub struct SetPairStatusKeys {
    pub lb_pair: Pubkey,
    pub admin: Pubkey,
}
impl From<&[Pubkey]> for SetPairStatusKeys {
    fn from(pubkeys: &[Pubkey]) -> Self {
        Self {
            lb_pair: pubkeys[0],
            admin: pubkeys[1],
        }
    }
}
impl SetPairStatusKeys {
    /// Convert Keys to Vec<Pubkey>
    pub fn to_vec(&self) -> Vec<Pubkey> {
        vec![self.lb_pair, self.admin]
    }
    /// Manual JSON serialization
    #[cfg(feature = "serde")]
    pub fn to_json(&self) -> String {
        format!(
            "{{{}}}",
            [
                format!("\"{}\":\"{}\"", "lb_pair", self.lb_pair),
                format!("\"{}\":\"{}\"", "admin", self.admin)
            ]
            .join(",")
        )
    }
}
impl From<SetPairStatusKeys>
    for [solana_instruction::AccountMeta; SET_PAIR_STATUS_IX_ACCOUNTS_LEN]
{
    fn from(keys: SetPairStatusKeys) -> Self {
        [
            solana_instruction::AccountMeta::new(keys.lb_pair, false),
            solana_instruction::AccountMeta::new(keys.admin, false),
        ]
    }
}
pub fn set_pair_status_ix_with_program_id(
    program_id: Pubkey,
    keys: SetPairStatusKeys,
    args: SetPairStatusIxData,
) -> Result<solana_instruction::Instruction, std::io::Error> {
    let metas: [solana_instruction::AccountMeta; SET_PAIR_STATUS_IX_ACCOUNTS_LEN] = keys.into();
    Ok(solana_instruction::Instruction {
        program_id,
        accounts: Vec::from(metas),
        data: args.try_to_vec()?,
    })
}
pub fn set_pair_status_ix(
    keys: SetPairStatusKeys,
    args: SetPairStatusIxData,
) -> Result<solana_instruction::Instruction, std::io::Error> {
    set_pair_status_ix_with_program_id(crate::ID, keys, args)
}

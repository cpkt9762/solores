...
pub fn try_unpack_account(data: &[u8]) -> Result<LbClmmAccount, std::io::Error> {
    match BinArray::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::BinArray(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match BinArrayBitmapExtension::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::BinArrayBitmapExtension(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match ClaimFeeOperator::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::ClaimFeeOperator(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match DummyZcAccount::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::DummyZcAccount(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match LbPair::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::LbPair(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match Oracle::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::Oracle(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    ...
    match Position::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::Position(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match PositionV2::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::PositionV2(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match PresetParameter::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::PresetParameter(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match PresetParameter2::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::PresetParameter2(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    ...
    match TokenBadge::from_bytes(data) {
        Ok(account) => return Ok(LbClmmAccount::TokenBadge(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }

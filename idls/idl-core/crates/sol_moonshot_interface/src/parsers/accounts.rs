...
pub fn try_unpack_account(data: &[u8]) -> Result<MoonshotAccount, std::io::Error> {
    match ConfigAccount::from_bytes(data) {
        Ok(account) => return Ok(MoonshotAccount::ConfigAccount(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match CurveAccount::from_bytes(data) {
        Ok(account) => return Ok(MoonshotAccount::CurveAccount(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }

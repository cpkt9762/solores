...
pub fn try_unpack_account(data: &[u8]) -> Result<RaydiumLaunchpadAccount, std::io::Error> {
    match GlobalConfig::from_bytes(data) {
        Ok(account) => return Ok(RaydiumLaunchpadAccount::GlobalConfig(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match PlatformConfig::from_bytes(data) {
        Ok(account) => return Ok(RaydiumLaunchpadAccount::PlatformConfig(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match PoolState::from_bytes(data) {
        Ok(account) => return Ok(RaydiumLaunchpadAccount::PoolState(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match VestingRecord::from_bytes(data) {
        Ok(account) => return Ok(RaydiumLaunchpadAccount::VestingRecord(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }

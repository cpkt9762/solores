//! This file was automatically generated by Solores
//! Solana IDL to Rust interface generator
//!
//! GitHub: https://github.com/cpkt9762/solores
//!
//! Generated on: 2025-08-16
//!
//! DO NOT EDIT - This file is auto-generated
//! Changes will be lost when regenerating from IDL

//!Instruction: ChangeFeeRecipient (NonAnchor)
#[allow(unused_imports)]
use solana_pubkey::Pubkey;
pub const CHANGE_FEE_RECIPIENT_IX_DISCM: u8 = 27u8;
pub const CHANGE_FEE_RECIPIENT_IX_ACCOUNTS_LEN: usize = 5;
#[derive(borsh::BorshDeserialize, borsh::BorshSerialize, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct ChangeFeeRecipientIxData {
    pub discriminator: u8,
}
impl Default for ChangeFeeRecipientIxData {
    fn default() -> Self {
        Self {
            discriminator: CHANGE_FEE_RECIPIENT_IX_DISCM,
        }
    }
}
impl ChangeFeeRecipientIxData {
    pub fn new() -> Self {
        Self {
            discriminator: Self::discriminator(),
        }
    }
    pub fn from_bytes(buf: &[u8]) -> std::io::Result<Self> {
        borsh::BorshDeserialize::deserialize(&mut &buf[..])
    }
    pub fn discriminator() -> u8 {
        CHANGE_FEE_RECIPIENT_IX_DISCM
    }
    pub fn try_to_vec(&self) -> std::io::Result<Vec<u8>> {
        borsh::to_vec(self)
    }
    /// Manual JSON serialization
    #[cfg(feature = "serde")]
    pub fn to_json(&self) -> String {
        format!(
            "{{{}}}",
            [format!("\"discriminator\":{}", self.discriminator)].join(",")
        )
    }
}
#[derive(Copy, Clone, Debug, PartialEq)]
pub struct ChangeFeeRecipientKeys {
    pub phoenix_program: Pubkey,
    pub log_authority: Pubkey,
    pub market: Pubkey,
    pub market_authority: Pubkey,
    pub new_fee_recipient: Pubkey,
}
impl Default for ChangeFeeRecipientKeys {
    fn default() -> Self {
        Self {
            phoenix_program: Pubkey::default(),
            log_authority: Pubkey::default(),
            market: Pubkey::default(),
            market_authority: Pubkey::default(),
            new_fee_recipient: Pubkey::default(),
        }
    }
}
impl From<&[Pubkey]> for ChangeFeeRecipientKeys {
    fn from(pubkeys: &[Pubkey]) -> Self {
        Self {
            phoenix_program: pubkeys[0],
            log_authority: pubkeys[1],
            market: pubkeys[2],
            market_authority: pubkeys[3],
            new_fee_recipient: pubkeys[4],
        }
    }
}
impl ChangeFeeRecipientKeys {
    /// Convert Keys to Vec<Pubkey>
    pub fn to_vec(&self) -> Vec<Pubkey> {
        vec![
            self.phoenix_program,
            self.log_authority,
            self.market,
            self.market_authority,
            self.new_fee_recipient,
        ]
    }
    /// Manual JSON serialization
    #[cfg(feature = "serde")]
    pub fn to_json(&self) -> String {
        format!(
            "{{{}}}",
            [
                format!(
                    "\"{}\":{}",
                    "phoenixProgram",
                    serde_json::to_string(&self.phoenix_program)
                        .unwrap_or_else(|_| "\"null\"".to_string())
                ),
                format!(
                    "\"{}\":{}",
                    "logAuthority",
                    serde_json::to_string(&self.log_authority)
                        .unwrap_or_else(|_| "\"null\"".to_string())
                ),
                format!(
                    "\"{}\":{}",
                    "market",
                    serde_json::to_string(&self.market).unwrap_or_else(|_| "\"null\"".to_string())
                ),
                format!(
                    "\"{}\":{}",
                    "marketAuthority",
                    serde_json::to_string(&self.market_authority)
                        .unwrap_or_else(|_| "\"null\"".to_string())
                ),
                format!(
                    "\"{}\":{}",
                    "newFeeRecipient",
                    serde_json::to_string(&self.new_fee_recipient)
                        .unwrap_or_else(|_| "\"null\"".to_string())
                )
            ]
            .join(",")
        )
    }
}
impl From<ChangeFeeRecipientKeys>
    for [solana_instruction::AccountMeta; CHANGE_FEE_RECIPIENT_IX_ACCOUNTS_LEN]
{
    fn from(keys: ChangeFeeRecipientKeys) -> Self {
        [
            solana_instruction::AccountMeta::new(keys.phoenix_program, false),
            solana_instruction::AccountMeta::new(keys.log_authority, false),
            solana_instruction::AccountMeta::new(keys.market, false),
            solana_instruction::AccountMeta::new(keys.market_authority, false),
            solana_instruction::AccountMeta::new(keys.new_fee_recipient, false),
        ]
    }
}
pub fn change_fee_recipient_ix_with_program_id(
    program_id: Pubkey,
    keys: ChangeFeeRecipientKeys,
) -> Result<solana_instruction::Instruction, std::io::Error> {
    let metas: [solana_instruction::AccountMeta; CHANGE_FEE_RECIPIENT_IX_ACCOUNTS_LEN] =
        keys.into();
    Ok(solana_instruction::Instruction {
        program_id,
        accounts: Vec::from(metas),
        data: ChangeFeeRecipientIxData::default().try_to_vec()?,
    })
}
pub fn change_fee_recipient_ix(
    keys: ChangeFeeRecipientKeys,
) -> Result<solana_instruction::Instruction, std::io::Error> {
    change_fee_recipient_ix_with_program_id(crate::ID, keys)
}

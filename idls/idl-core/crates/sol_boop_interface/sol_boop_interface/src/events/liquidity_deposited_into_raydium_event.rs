//! This file was automatically generated by Solores
//! Solana IDL to Rust interface generator
//!
//! GitHub: https://github.com/cpkt9762/solores
//!
//! Generated on: 2025-08-16
//!
//! DO NOT EDIT - This file is auto-generated
//! Changes will be lost when regenerating from IDL

//!Event: LiquidityDepositedIntoRaydiumEvent
#[allow(unused_imports)]
use solana_pubkey::Pubkey;
pub const LIQUIDITY_DEPOSITED_INTO_RAYDIUM_EVENT_EVENT_DISCM: [u8; 8] =
    [236u8, 50u8, 97u8, 27u8, 198u8, 101u8, 248u8, 20u8];
#[derive(borsh::BorshDeserialize, borsh::BorshSerialize, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct LiquidityDepositedIntoRaydiumEvent {
    pub discriminator: [u8; 8],
    #[cfg_attr(
        feature = "serde",
        serde(with = "serde_with::As::<serde_with::DisplayFromStr>")
    )]
    pub pool_state: Pubkey,
    #[cfg_attr(
        feature = "serde",
        serde(with = "serde_with::As::<serde_with::DisplayFromStr>")
    )]
    pub mint: Pubkey,
    pub lp_token_amount: u64,
    pub tokens_deposited: u64,
    pub wsol_deposited: u64,
}
impl Default for LiquidityDepositedIntoRaydiumEvent {
    fn default() -> Self {
        Self {
            discriminator: LIQUIDITY_DEPOSITED_INTO_RAYDIUM_EVENT_EVENT_DISCM,
            pool_state: Default::default(),
            mint: Default::default(),
            lp_token_amount: Default::default(),
            tokens_deposited: Default::default(),
            wsol_deposited: Default::default(),
        }
    }
}
impl LiquidityDepositedIntoRaydiumEvent {
    pub fn try_to_vec(&self) -> std::io::Result<Vec<u8>> {
        borsh::to_vec(self)
    }
    pub fn from_bytes(data: &[u8]) -> Result<Self, std::io::Error> {
        borsh::BorshDeserialize::deserialize(&mut &data[..])
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()))
    }
}

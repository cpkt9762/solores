//! This file was automatically generated by Solores
//! Solana IDL to Rust interface generator
//!
//! GitHub: https://github.com/cpkt9762/solores
//!
//! Generated on: 2025-08-16
//!
//! DO NOT EDIT - This file is auto-generated
//! Changes will be lost when regenerating from IDL

//!Instruction: cancel_authority_transfer
#[allow(unused_imports)]
use solana_pubkey::Pubkey;
pub const CANCEL_AUTHORITY_TRANSFER_IX_DISCM: [u8; 8] =
    [94u8, 131u8, 125u8, 184u8, 183u8, 24u8, 125u8, 229u8];
pub const CANCEL_AUTHORITY_TRANSFER_IX_ACCOUNTS_LEN: usize = 3;
#[derive(borsh::BorshDeserialize, borsh::BorshSerialize, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct CancelAuthorityTransferIxData {
    pub discriminator: [u8; 8],
}
impl Default for CancelAuthorityTransferIxData {
    fn default() -> Self {
        Self {
            discriminator: CANCEL_AUTHORITY_TRANSFER_IX_DISCM,
        }
    }
}
impl CancelAuthorityTransferIxData {
    pub fn new() -> Self {
        Self {
            discriminator: Self::discriminator(),
        }
    }
    pub fn from_bytes(buf: &[u8]) -> std::io::Result<Self> {
        borsh::BorshDeserialize::deserialize(&mut &buf[..])
    }
    pub fn discriminator() -> [u8; 8] {
        CANCEL_AUTHORITY_TRANSFER_IX_DISCM
    }
    pub fn try_to_vec(&self) -> std::io::Result<Vec<u8>> {
        borsh::to_vec(self)
    }
    /// Manual JSON serialization
    #[cfg(feature = "serde")]
    pub fn to_json(&self) -> String {
        format!(
            "{{{}}}",
            [format!(
                "\"discriminator\":[{}]",
                self.discriminator
                    .iter()
                    .map(|x| x.to_string())
                    .collect::<Vec<_>>()
                    .join(",")
            )]
            .join(",")
        )
    }
}
#[derive(Copy, Clone, Debug, PartialEq, Default)]
pub struct CancelAuthorityTransferKeys {
    pub authority: Pubkey,
    pub config: Pubkey,
    pub system_program: Pubkey,
}
impl From<&[Pubkey]> for CancelAuthorityTransferKeys {
    fn from(pubkeys: &[Pubkey]) -> Self {
        Self {
            authority: pubkeys[0],
            config: pubkeys[1],
            system_program: pubkeys[2],
        }
    }
}
impl CancelAuthorityTransferKeys {
    /// Convert Keys to Vec<Pubkey>
    pub fn to_vec(&self) -> Vec<Pubkey> {
        vec![self.authority, self.config, self.system_program]
    }
    /// Manual JSON serialization
    #[cfg(feature = "serde")]
    pub fn to_json(&self) -> String {
        format!(
            "{{{}}}",
            [
                format!("\"{}\":\"{}\"", "authority", self.authority),
                format!("\"{}\":\"{}\"", "config", self.config),
                format!("\"{}\":\"{}\"", "system_program", self.system_program)
            ]
            .join(",")
        )
    }
}
impl From<CancelAuthorityTransferKeys>
    for [solana_instruction::AccountMeta; CANCEL_AUTHORITY_TRANSFER_IX_ACCOUNTS_LEN]
{
    fn from(keys: CancelAuthorityTransferKeys) -> Self {
        [
            solana_instruction::AccountMeta::new(keys.authority, false),
            solana_instruction::AccountMeta::new(keys.config, false),
            solana_instruction::AccountMeta::new(keys.system_program, false),
        ]
    }
}
pub fn cancel_authority_transfer_ix_with_program_id(
    program_id: Pubkey,
    keys: CancelAuthorityTransferKeys,
) -> Result<solana_instruction::Instruction, std::io::Error> {
    let metas: [solana_instruction::AccountMeta; CANCEL_AUTHORITY_TRANSFER_IX_ACCOUNTS_LEN] =
        keys.into();
    let args = CancelAuthorityTransferIxData::default();
    Ok(solana_instruction::Instruction {
        program_id,
        accounts: Vec::from(metas),
        data: args.try_to_vec()?,
    })
}
pub fn cancel_authority_transfer_ix(
    keys: CancelAuthorityTransferKeys,
) -> Result<solana_instruction::Instruction, std::io::Error> {
    cancel_authority_transfer_ix_with_program_id(crate::ID, keys)
}

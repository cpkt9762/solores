...
pub fn try_unpack_account(data: &[u8]) -> Result<BoopAccount, std::io::Error> {
    match AmmConfig::from_bytes(data) {
        Ok(account) => return Ok(BoopAccount::AmmConfig(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match BondingCurve::from_bytes(data) {
        Ok(account) => return Ok(BoopAccount::BondingCurve(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    ...
    match Config::from_bytes(data) {
        Ok(account) => return Ok(BoopAccount::Config(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match LockedCpLiquidityState::from_bytes(data) {
        Ok(account) => return Ok(BoopAccount::LockedCpLiquidityState(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }

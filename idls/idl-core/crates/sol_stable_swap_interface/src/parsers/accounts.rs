...
pub fn try_unpack_account(data: &[u8]) -> Result<StableSwapAccount, std::io::Error> {
    match Pool::from_bytes(data) {
        Ok(account) => return Ok(StableSwapAccount::Pool(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match Strategy::from_bytes(data) {
        Ok(account) => return Ok(StableSwapAccount::Strategy(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }
    }
    match Vault::from_bytes(data) {
        Ok(account) => return Ok(StableSwapAccount::Vault(account)),
        Err(AccountParseError::DiscriminatorMismatch { .. }) => {}
        Err(e) => {
            return Err(e.into());
        }

//! This file was automatically generated by Solores
//! Solana IDL to Rust interface generator
//!
//! GitHub: https://github.com/cpkt9762/solores
//!
//! Generated on: 2025-08-16
//!
//! DO NOT EDIT - This file is auto-generated
//! Changes will be lost when regenerating from IDL

//!Instruction: transfer_owner
#[allow(unused_imports)]
use solana_pubkey::Pubkey;
pub const TRANSFER_OWNER_IX_DISCM: [u8; 8] = [245u8, 25u8, 221u8, 175u8, 106u8, 229u8, 225u8, 45u8];
pub const TRANSFER_OWNER_IX_ACCOUNTS_LEN: usize = 2;
#[derive(borsh::BorshDeserialize, borsh::BorshSerialize, Clone, Debug, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct TransferOwnerIxData {
    pub discriminator: [u8; 8],
    #[cfg_attr(
        feature = "serde",
        serde(with = "serde_with::As::<serde_with::DisplayFromStr>")
    )]
    pub new_owner: Pubkey,
}
impl Default for TransferOwnerIxData {
    fn default() -> Self {
        Self {
            discriminator: TRANSFER_OWNER_IX_DISCM,
            new_owner: Default::default(),
        }
    }
}
impl TransferOwnerIxData {
    pub fn new(new_owner: Pubkey) -> Self {
        Self {
            discriminator: Self::discriminator(),
            new_owner,
        }
    }
    pub fn from_bytes(buf: &[u8]) -> std::io::Result<Self> {
        borsh::BorshDeserialize::deserialize(&mut &buf[..])
    }
    pub fn discriminator() -> [u8; 8] {
        TRANSFER_OWNER_IX_DISCM
    }
    pub fn try_to_vec(&self) -> std::io::Result<Vec<u8>> {
        borsh::to_vec(self)
    }
    /// Manual JSON serialization
    #[cfg(feature = "serde")]
    pub fn to_json(&self) -> String {
        format!(
            "{{{}}}",
            [
                format!(
                    "\"discriminator\":[{}]",
                    self.discriminator
                        .iter()
                        .map(|x| x.to_string())
                        .collect::<Vec<_>>()
                        .join(",")
                ),
                format!("\"{}\":{}", "new_owner", self.new_owner)
            ]
            .join(",")
        )
    }
}
#[derive(Copy, Clone, Debug, PartialEq, Default)]
pub struct TransferOwnerKeys {
    pub owner: Pubkey,
    pub pool: Pubkey,
}
impl From<&[Pubkey]> for TransferOwnerKeys {
    fn from(pubkeys: &[Pubkey]) -> Self {
        Self {
            owner: pubkeys[0],
            pool: pubkeys[1],
        }
    }
}
impl TransferOwnerKeys {
    /// Convert Keys to Vec<Pubkey>
    pub fn to_vec(&self) -> Vec<Pubkey> {
        vec![self.owner, self.pool]
    }
    /// Manual JSON serialization
    #[cfg(feature = "serde")]
    pub fn to_json(&self) -> String {
        format!(
            "{{{}}}",
            [
                format!("\"{}\":\"{}\"", "owner", self.owner),
                format!("\"{}\":\"{}\"", "pool", self.pool)
            ]
            .join(",")
        )
    }
}
impl From<TransferOwnerKeys> for [solana_instruction::AccountMeta; TRANSFER_OWNER_IX_ACCOUNTS_LEN] {
    fn from(keys: TransferOwnerKeys) -> Self {
        [
            solana_instruction::AccountMeta::new(keys.owner, false),
            solana_instruction::AccountMeta::new(keys.pool, false),
        ]
    }
}
pub fn transfer_owner_ix_with_program_id(
    program_id: Pubkey,
    keys: TransferOwnerKeys,
    args: TransferOwnerIxData,
) -> Result<solana_instruction::Instruction, std::io::Error> {
    let metas: [solana_instruction::AccountMeta; TRANSFER_OWNER_IX_ACCOUNTS_LEN] = keys.into();
    Ok(solana_instruction::Instruction {
        program_id,
        accounts: Vec::from(metas),
        data: args.try_to_vec()?,
    })
}
pub fn transfer_owner_ix(
    keys: TransferOwnerKeys,
    args: TransferOwnerIxData,
) -> Result<solana_instruction::Instruction, std::io::Error> {
    transfer_owner_ix_with_program_id(crate::ID, keys, args)
}
